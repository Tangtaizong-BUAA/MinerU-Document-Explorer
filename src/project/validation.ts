...
export function validateProjectRecords(records: KnowledgeRecord[]): ValidationIssue[] {
  ...
  for (const record of records) {
    ...
    if (record.type === "memory" && record.status === "accepted" && (!present(record.evidence_refs) || !present(record.validation_event_ref))) issues.push({ code: "accepted_memory_missing_validation", record_id: record.id, message: "Accepted memory requires evidence_refs and validation_event_ref", severity: "error" });
