...
> 设计版本：0.5.0
> 当前生产版本：0.5.0；阿里 live gate、家庭影子门禁、正式主节点与阿里只读回退均已通过
> 生效日期：2026-07-29
...
机器可读契约见 [specs/mcp-surface.v0.5.yaml](specs/mcp-surface.v0.5.yaml)。0.4 历史 spec 不改写；2026-07-30 已核验实际工具面、Skill 与 0.5 镜像一致。
...
0.5 的百炼配置冻结为华北 2（北京）兼容接口。2026-07-29 已复用 ArgonType 受管环境中的既有 Key，通过北京区 OpenAI-compatible endpoint 实测 `qwen3.7-flash` 与 `qwen3-vl-embedding` 成功。配置必须把 `provider_region=cn-beijing`、endpoint、API dialect、模型 ID 和 key reference 作为同一版本化 tuple；Key 与 endpoint 不得跨地域混用。未来切换业务空间专属 endpoint、Key 或地域时必须重跑 live gate。
...
发布仍遵守同一门禁：只有代码、迁移、回归、影子路径、家庭主节点和阿里回退全部验收后，才能发布同版本 Server 与 Skill。0.5.0 已于 2026-07-30 按该流程发布；各分布式客户端继续通过版本握手、下一任务/重启激活和 legacy shim 分阶段切换，不假设所有客户端瞬时原子升级。generated bundle 只能通过现有同步脚本生成，不能手改。
...
第二项百炼地域已经闭合：现有 ArgonType Key 与北京区兼容 endpoint 的真实调用成功。Key 继续只保存在受管环境，不进入仓库；任何 endpoint、地域或 Key 变更都重新验收。

2026-07-29 live gate 已完成：`qwen3.7-flash` 文本与工具调用、图片、原生 `video_url`，完整 MS-Agent 结构化维护计划，以及 `qwen3-vl-embedding` 文本/图片/视频 1024 维向量均通过。测试只使用合成素材和阿里官方公开媒体，没有外发项目资料。原始音频理解仍需另行选择 Omni/ASR 模型。
